...
> **원칙**: 토큰 값·스펙·구현 참조만 기록한다. 행동 규칙(테마 정의·금지 패턴)은 `design-principles.md` 참조.
> CSS 구현: `src/styles/` (글로벌 5파일) + `src/styles/pages/` (라우트별 1:1) + `src/components/**/*.module.css`
...
### `--color-text` 계열 — Liquid Glass UI Kit 전용 텍스트 토큰

> **`--text-primary`와 공존하는 별도 계열**. CSS 파일(`.css`)에서만 사용한다. TSX 인라인 스타일에서는 `--text-primary` / `--text-secondary` 사용.

#### 왜 두 계열이 존재하는가

`--text-primary`는 앱 Shell 전반에서 크림 화이트(`#eff4ff`) 색조를 유지한다.
`--color-text`는 Liquid Glass 표면(강한 blur + 반투명 배경) 위에서 가독성을 높이기 위해
다크에서 순백(`#ffffff`)으로 설계된 별도 계열이다. Guide 컴포넌트 CSS에서 이 계열을 사용한다.

| 토큰 | 다크 | 라이트 | 용도 |
|------|------|--------|------|
| `--color-text` | `#ffffff` | `#0f172a` | 본문 — glass 표면 위 주 텍스트 |
| `--color-text-muted` | `rgba(255,255,255,0.55)` | `rgba(15,23,42,0.58)` | 보조 — 설명·캡션 |
| `--color-text-subtle` | `rgba(255,255,255,0.35)` | `rgba(15,23,42,0.38)` | 3차 — 부가 정보·힌트 |

#### `--text-primary` vs `--color-text` 사용 기준

| 상황 | 사용할 토큰 |
|------|-----------|
| TSX `style={{ color: ... }}` 인라인 | `var(--text-primary)` / `var(--text-secondary)` |
| `.css` 파일 (Guide·landing·homepage CSS) | `var(--color-text)` / `var(--color-text-muted)` / `var(--color-text-subtle)` |
| 신규 CSS 클래스 작성 | `--color-text` 계열 (3단계 계층 활용) |
| 신규 TSX 컴포넌트 인라인 스타일 | `--text-primary` 계열 |

> `--color-text` 정의 위치: `apps/web/src/styles/tokens.css` `:root` 블록 (Liquid Glass 공통 토큰 섹션)
...
`landing1`, `landing2`, `landing3`, `landing4`, `landing7` — `LdGnbConditional.tsx`에서 `forceDarkGnb=true` 전달.
다른 페이지 추가 시 `LdGnbConditional.tsx`의 `forceDarkGnb` 조건에 pathname 추가 필요.
상세 규칙: `rules/gnb.md` §7 참조.

---

## 11. 시네마틱 랜딩 시스템 (`components/landing/cinematic/LdCinematicLanding.css`)
...
CSS 파일 위치: `apps/web/src/components/landing/cinematic/LdCinematicLanding.css` (컴포넌트 동거)
활성 조건: `landings.json`에서 해당 slug의 `variant: "cinematic"` 설정 시
