...
const STATUS_COLORS: Record<string, string> = { complete: "#10b981", pending: "#eab308", refunded: "#ef4444" };
const STATUS_BG: Record<string, string> = { complete: "rgba(16,185,129,0.12)", pending: "rgba(234,179,8,0.12)", refunded: "rgba(239,68,68,0.12)" };

export default function OrdersPage() {
  const [statusFilter, setStatusFilter] = useState<OrderStatus>("all");
  const [expandedId, setExpandedId] = useState<string | null>(null);

  const filtered = mockOrders.filter((o) => statusFilter === "all" ? true : o.status === statusFilter);
                  ...
                  <tr key={order.id} style={{ borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                    ...
                    <td style={{ ...tdStyle, textAlign: "right" }}>
                      <button
                        ...
                        style={{ padding: "4px 10px", borderRadius: 6, backgroundColor: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.1)", color: "var(--text-secondary)", fontSize: 13, cursor: "pointer", fontFamily: "'Pretendard Variable', 'Pretendard', sans-serif" }}
                      >
