...
import { faTimes, faChevronRight, faLightbulb, faArrowLeft } from '@fortawesome/free-solid-svg-icons';
...
const CATEGORIES = ['전체', '링크브라우저 이미지 제작', 'MP4 제작', '훅 전략', 'Google Flow', '직접 촬영', '썸네일 연계', '업로드 팁'];

const CAT_BADGE: Record<string, string> = {
  '링크브라우저 이미지 제작': 'bg-indigo-500/10 text-indigo-400 border-indigo-500/20',
  'MP4 제작':               'bg-fuchsia-500/10 text-fuchsia-400 border-fuchsia-500/20',
...
const CAT_ACCENT: Record<string, string> = {
  '링크브라우저 이미지 제작': 'from-indigo-400 to-blue-400',
  'MP4 제작':               'from-fuchsia-400 to-pink-400',
...
export default function KeyframeTipsPage() {
  ...
  const handleBack = (e: React.MouseEvent) => {
    e.preventDefault();
    setIsLeaving(true);
    setTimeout(() => router.push('/content/keyframe'), 280);
  };
  ...
  return (
    <div className={`min-h-screen bg-[#050B1E] text-white font-sans overflow-x-hidden ${isLeaving ? 'page-slide-out' : 'page-slide-in'}`}>
        ...
        <div className="max-w-7xl mx-auto relative z-10 flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
          <div>
            <nav className="flex items-center gap-2 text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-3">
              ...
              <span>›</span>
              <Link href="/content/keyframe" className="hover:text-cyan-400 transition-colors">키프레임</Link>
              <span>›</span>
          ...
          <div className="flex items-center gap-3 flex-shrink-0">
            ...
            <button
              onClick={() => { setIsLeaving(true); setTimeout(() => router.push('/content/mp4-prompt-generator'), 280); }}
              className="flex items-center gap-2 px-4 py-2.5 bg-gradient-to-r from-rose-600 to-amber-500 hover:brightness-110 rounded-xl text-xs font-black text-white transition-all shadow-lg shadow-rose-500/20 active:scale-95"
            >
              <span>🎬</span>
              MP4 영상 만들기
            </button>
