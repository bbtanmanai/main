...
씬 구성 예시:
[씬1] 오프닝 훅 — 1~2문장.
[씬2] 도입부 — "내 이름은 ${name || '우진'}이야. 만약 네가 [시청자의 고민] 때문에 힘들다면 구독 눌러줘."
...
// ── StepNav ───────────────────────────────────────────────────────────────────
function JunStepNav({ active }: { active: string }) {
  const steps = [
...
// ── Main ──────────────────────────────────────────────────────────────────────
export default function Jun1Page() {
  const [step, setStep] = React.useState(1);

  // Step 1
  const [url, setUrl] = React.useState('');
  const [channelName, setChannelName] = React.useState('우진');
        ...
        <div className="max-w-6xl mx-auto relative z-10">
          ...
          <nav aria-label="Breadcrumb" className="mb-5 flex justify-end">
            <ol className="flex items-center gap-2 text-sm flex-wrap">
              ...
              <li aria-hidden="true"><FontAwesomeIcon icon={faChevronRight} className="text-gray-300 text-[10px]" /></li>
              <li><span className="text-violet-600 font-black" aria-current="page">전우진</span></li>
            </ol>
          </nav>

          <div className="mb-6">
            <JunStepNav active="jun-1" />
          </div>
          ...
          <div className="flex items-center gap-4 mb-2">
            ...
            <h1 className="text-4xl font-black tracking-tighter text-gray-800">
              전우진 <span className="bg-gradient-to-r from-violet-600 to-purple-800 bg-clip-text text-transparent">× 대본 생산</span>
            </h1>
          </div>
                  ...
                  <div className="flex items-center justify-center gap-4">
                    ...
                    <button
                      onClick={() => {
                        ...
                        window.location.href = '/team/jun-2';
                      }}
